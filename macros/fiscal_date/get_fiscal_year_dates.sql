...
{% macro maxcompute__get_fiscal_year_dates(dates, year_end_month, week_start_day, shift_year) %}
-- this gets all the dates within a fiscal year
-- determined by the given year-end-month
-- ending on the saturday closest to that month's end date
with fsc_date_dimension as (
...
year_month_end as (
    ...
    where
        d.month_of_year = {{ year_end_month }}
    group by fiscal_year_number, d.month_end_date
...
year_week_ends as (
    ...
    where
        d.month_of_year = {{ year_end_month }}
    group by
        fiscal_year_number,d.week_end_date
